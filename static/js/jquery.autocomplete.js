...
(function($) {
    $.fn.extend({
        autocomplete: function(urlOrData, options) {
            ...
            $.Autocompleter.defaults({ url: isUrl ? urlOrData : null, data: isUrl ? null : urlOrData, delay: isUrl ? $.Autocompleter.defaults.delay : 10, max: options && !options.scroll ? 10 : 150 }, options);
    ...
    $.Autocompleter = function(input, options) {
        ...
        }).bind("flushCache", function() { cache.flush() }).bind("setOptions", function() { $.extend(options, arguments[1]); if ("data" in arguments[1]) cache.populate() }).bind("unautocomplete", function() {
            select.unbind();
            $input.unbind();
            ...
            if (options.multiple) {
                ...
                if (words.length > 1) {
                    ...
                    $.each(words, function(i, word) {
                        ...
                        if (cursorAt <= progress) { wordAt = i; return false }
                        progress += seperator
                    });
                    words[wordAt] = v;
        ...
        function request(term, success, failure) {
            ...
            if (data && data.length) { success(term, data) } else if ((typeof options.url == "string") && (options.url.length > 0)) {
                ...
                $.ajax({
                    ...
                    url: options.url,
                    data: $.extend({ wd: lastWord(term), limit: options.max }, extraParams),
    ...
    $.Autocompleter.defaults = { selectFirst: true, formatItem: function(row) { return row[0] }, formatMatch: null, autoFill: false, width: 0, multiple: false, multipleSeparator: ", ", highlight: function(value, term) { return value.replace(new RegExp("(?![^&;]+;)(?!<[^<>]*)(" + term.replace(/([\^\$\(\)\[\]\{\}\*\.\+\?\|\\])/gi, "\\$1") + ")(?![^<>]*>)(?![^&;]+;)", "gi"), "<strong>$1</strong>") }, scroll: true, scrollHeight: 180 };
        ...
        return {
            ...
            load: function(q) {
                ...
                if (!options.url && options.matchContains) {
                    ...
                    return csub
                } else if (data[q]) { return data[q] } else if (options.matchSubset) {
    ...
    $.Autocompleter.Select = function(options, input, select, config) {
        ...
        function moveSelect(step) {
            ...
            if (options.scroll) {
                ...
                if ((offset + activeItem[0].offsetHeight - list.scrollTop()) > list[0].clientHeight) { list.scrollTop(offset + activeItem[0].offsetHeight - list.innerHeight()) } else if (offset < list.scrollTop()) { list.scrollTop(offset) }
            }
        };
    ...
    $.fn.selection = function(start, end) {
        if (start !== undefined) {
            return this.each(function() {
                ...
                } else if (this.setSelectionRange) { this.setSelectionRange(start, end) } else if (this.selectionStart) {
        ...
        } else if (field.selectionStart !== undefined) { return { start: field.selectionStart, end: field.selectionEnd } }
    }
})(jQuery);